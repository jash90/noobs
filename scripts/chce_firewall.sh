...
if [[ -n "$host_num" ]]; then
    ...
    msg_warn "Nie udało się automatycznie ustalić numeru hosta. Pomijam dodawanie portów Mikrus."
fi
...
if [ "$#" -gt 0 ]; then
    ...
    for extra_port in "$@"; do
        if [[ "$extra_port" =~ ^[0-9]+$ ]] && (( extra_port >= 1 && extra_port <= 65535 )); then
            ...
            msg_warn "Argument '$extra_port' nie jest poprawnym numerem portu (1-65535) i został zignorowany."
        fi
    done
fi
...
if ! sudo iptables -C INPUT -m state --state INVALID -j DROP &>/dev/null; then
    ...
    msg_info "Reguły iptables do ochrony przed skanowaniem już istnieją."
fi

msg_ok "Konfiguracja firewalla zakończona."
