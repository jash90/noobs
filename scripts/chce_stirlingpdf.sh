...
EOF
  ...
  service_enable stirlingpdf.service
}
...
install_stirling_pdf() {
  ...
  service_start stirlingpdf.service || { msg_error "Nie udało się uruchomić usługi"; exit 1; }
  service_restart stirlingpdf.service || { msg_error "Nie udało się uruchomić usługi"; exit 1; }


  cleanup_temp_files
  local port="${CUSTOM_PORT:-8080}"
  msg_ok "Instalacja ${APP} zakończona pomyślnie!"
  msg_info "Aplikacja będzie dostępna pod adresem: http://$(get_local_ip):${port}"
}
...
update_stirling_pdf() {
  ...
  if check_for_updates; then
    ...
    msg_info "Aplikacja jest dostępna pod adresem: http://$(get_local_ip):${port}"
  else
...
get_installed_version() {
  ...
  if service_is_active stirlingpdf.service; then
...
remove_stirling_pdf() {
  ...
  service_stop stirlingpdf.service 2>/dev/null || true
  service_disable stirlingpdf.service 2>/dev/null || true
...
restart_application() {
  ...
  service_restart stirlingpdf.service || { msg_error "Nie udało się zrestartować usługi"; exit 1; }
